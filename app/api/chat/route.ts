    ...
    tools: {
      getSpendingByCategory: tool({
        ...
        execute: async ({ startDate, endDate, limit }) => {
          ...
          if (!data || data.length === 0) {
            return { categories: [], message: 'No spending data found for this period' }
          }
      ...
      getMonthlySpending: tool({
        ...
        execute: async ({ months }) => {
          ...
          data.forEach((t) => {
            ...
            monthlyTotals[month] = (monthlyTotals[month] || 0) + Math.abs(Number(t.amount))
          })
      ...
      searchTransactions: tool({
        ...
        execute: async ({ query, limit }) => {
          ...
          const { data } = await supabase
            .from('transactions')
            .select('date, description, merchant, amount, category, transaction_type')
            .or(`description.ilike.%${query}%,merchant.ilike.%${query}%`)
            .order('date', { ascending: false })
            .limit(limit)

          if (!data || data.length === 0) {
            return { transactions: [], message: `No transactions found matching "${query}"` }
          }
      ...
      compareMonths: tool({
        ...
        execute: async ({ month1, month2 }) => {
          ...
          const percentChange = data1.total > 0 
