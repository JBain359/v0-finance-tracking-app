...
function parseCSV(content: string): ParsedTransaction[] {
  ...
  for (const row of result.data as Record<string, string>[]) {
    ...
    const dateValue = row['date'] || row['transaction date'] || row['post date'] || row['posting date'] || Object.values(row)[0]
    
    // Try to find description column
    const description = row['description'] || row['memo'] || row['merchant'] || row['name'] || row['payee'] || Object.values(row)[1] || ''
    ...
    const debit = parseFloat((row['debit'] || row['withdrawal'] || row['amount debit'] || '0').replace(/[^0-9.-]/g, ''))
    const credit = parseFloat((row['credit'] || row['deposit'] || row['amount credit'] || '0').replace(/[^0-9.-]/g, ''))
    ...
    if (debit && !isNaN(debit) && debit !== 0) {
      ...
      const amountStr = row['amount'] || row['transaction amount'] || Object.values(row)[2] || '0'
    ...
    if (parsedDate && !isNaN(parsedDate.getTime()) && description && amount > 0) {
      transactions.push({
  ...
  try {
    ...
    if (statement.file_type === 'csv') {
      ...
      return NextResponse.json({ 
        error: 'PDF parsing is not yet fully supported. Please use CSV files.' 
      }, { status: 400 })
    }
    ...
    const transactionsToInsert = transactions.map((t) => {
      const { category, merchant } = categorizeTransaction(t.description, categoryList)
      return {
